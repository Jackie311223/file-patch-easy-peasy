...
import { useAuth } from '../hooks/useAuth'; // Assuming auth hook provides user info/token
  ...
  const onSubmit = async (data: RoomTypeFormData) => {
    ...
    try {
      if (editingRoomType) {
        ...
        await api.put(`/properties/${propertyId}/room-types/${editingRoomType.id}`, payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
        // Create new room type
        await api.post(`/properties/${propertyId}/room-types`, payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
